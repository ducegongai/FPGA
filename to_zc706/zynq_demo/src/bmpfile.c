...
FILE *BMP_InputOpen(char *name) {
    ...
    if(!bmp_file) printf("ERROR: could not open %s for reading\n", name);
    else return bmp_file;
}

// Open the BMP output file
FILE *BMP_OutputOpen(char *name) {
    FILE *bmp_file = fopen(name,"wb");
    if(!bmp_file) printf("ERROR: could not open %s for writing\n", name);
    else return bmp_file;
}
...
int BMPReadAd(char *file, unsigned char **R, unsigned char **G, unsigned char **B, int *prow, int *pcol) {
    ...
    *pcol = (int)(image_header->Width);
    ...
    // malloc memory space
    *R = (unsigned char*)malloc(row * col * sizeof(unsigned char));
    *G = (unsigned char*)malloc(row * col * sizeof(unsigned char));
    *B = (unsigned char*)malloc(row * col * sizeof(unsigned char));
...
// Convert RGB to Grey
int rgb2gray(unsigned char *R, unsigned char *G, unsigned char *B, int nrows, int ncols, unsigned char *grey) {
    int i, j, index;
    for(i = 0; i < nrows; i++) {
        for(j = 0; j < ncols; j++) {
            index = i *ncols + j;
            unsigned char temp_R, temp_G, temp_B, temp_gray;
            temp_R = R[index];
            temp_G = G[index];
            temp_B = B[index];
            temp_gray = ((66 * temp_R + 129 * temp_G + 25 * temp_B +128) >> 8) + 16;
            grey[index] = temp_gray;
        }
    }
    return 0;
}
...
// Load RGB imgage into grey
void LoadGrayImg(char *file, int ncols, int nrows, unsigned char *grey) {
    unsigned char *R, *G, *B;
    R = (unsigned char *)malloc(ncols*nrows*sizeof(int));
    G = (unsigned char *)malloc(ncols*nrows*sizeof(int));
    B = (unsigned char *)malloc(ncols*nrows*sizeof(int));
    BMP_Read(file, nrows, ncols, R, G, B);
    rgb2gray(R, G, B, nrows, ncols, grey);
    free(R);
    free(G);
    free(B);
}
